...
use crate::common::pools::{Pool, load_all_pools};
...
pub async fn run_sandwich_strategy(provider: Arc<Provider<Ws>>, event_sender: Sender<Event>) {
    ...
    let (pools, prev_pool_id) = load_all_pools(env.wss_url.clone(), 10000000, 50000).await.unwrap();

    // load_all_tokens:
    // this will get all the token information including: name, symbol, symbol, totalSupply
    let block_number = provider.get_block_number().await.unwrap();
    let tokens_map = load_all_tokens(&provider, block_number, &pools, prev_pool_id).await.unwrap();
            ...
            Ok(event) =>
                match event {
                    ...
                    Event::PendingTx(mut pending_tx) => {
                        let swap_info = extract_swap_info(
                            &provider,
                            &new_block,
                            &pending_tx,
                            &pools_map
